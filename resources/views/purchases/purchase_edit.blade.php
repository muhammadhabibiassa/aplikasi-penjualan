    ...
    <div class="row">
      <div class="col-md-12">
        <div class="card p-3 m-3">
          ...
          <form method="POST" action="{{route('purchase.update', ['id' =>$data->id])}}">
            @csrf
            <div class="row">
              <div class="col-md-6 mb-3">
                ...
                <select class="form-control" name="idSupplier">
                  ...
                  <option value="{{$supplier->id}}" {{$supplier->id === $data->idSupplier? 'selected' : ''}}>{{$supplier->name}}</option>
                  @endforeach
                </select>
              </div>
              <div class="col-md-6 mb-3">
              ...
              <div class="col-md-6 mb-3">
                <label>Date {{ date('m/d/Y', strtotime($data->date)) }}</label>
                <input type="datetime-local" class="form-control" name="date" value="{{ date('m/d/Y', strtotime($data->date)) }}">
              </div>
              <div class="col-md-6 mb-3">
                <label>Status</label>
                <label class="switch">
                  <input type="checkbox" {{$data->status == 1? 'checked' : ''}} value="{{$data->status}}" name="status">
                      ...
                      @if (count($items) > 0)
                        @foreach ($items as $data)
                          <tr>
                            ...
                            <td>{{$data->quantity}}</td>
                            <td><input type="number" class="form-control" name="ordered[]" value="{{$data->id}}" /></td>
                          </tr>
                        @endforeach
                        @else
                        <tr>
